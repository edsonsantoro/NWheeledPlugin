...
#define LOCTEXT_NAMESPACE "UNDWheeledVehicleMovementComponent"
...
*/
void PTireShader2(const void* shaderData, const PxF32 tireFriction,
	...
	PxF32& wheelTorque, PxF32& tireLongForceMag, PxF32& tireLatForceMag, PxF32& tireAlignMoment)
{
	...
	FTireShaderOutputND Output(0.0f);
	
	Wheel->VehicleNDSim->GenerateTireForces(Wheel, Input, Output);
...
UNDWheeledVehicleMovementComponent::UNDWheeledVehicleMovementComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UNDWheeledVehicleMovementComponent::SetupWheels(PxVehicleWheelsSimData* PWheelsSimData)
{
	if (!UpdatedPrimitive)
	{
		return;
...
PX_FORCE_INLINE PxF32 smoothingFunction3(const PxF32 K)
{
...
PX_FORCE_INLINE PxF32 smoothingFunction4(const PxF32 K)
{
...
void PxVehicleComputeTireForceDefault2
...
PxF32& wheelTorque, PxF32& tireLongForceMag, PxF32& tireLatForceMag, PxF32& tireAlignMoment)
{
	...
	const PxF32 latStiff = restTireLoad*tireData.mLatStiffY*smoothingFunction3(normalisedTireLoad*3.0f / tireData.mLatStiffX);
	...
	PxF32 FBar = smoothingFunction3(K);//K - ONE_THIRD*PxAbs(K)*K + ONE_TWENTYSEVENTH*K*K*K;
	PxF32 MBar = smoothingFunction4(K); //K - KAbs*K + ONE_THIRD*K*K*K - ONE_TWENTYSEVENTH*KAbs*K*K*K;
...
void UNDWheeledVehicleMovementComponent::GenerateTireForces(UVehicleWheelWithCamber* Wheel, const FTireShaderInputND& Input, FTireShaderOutputND& Output)
{
	const void* realShaderData = &PVehicle->mWheelsSimData.getTireData(Wheel->WheelIndex);

	float Dummy;

	PxVehicleComputeTireForceDefault2(
